    ...
    <c:choose>
        <c:when test="${! empty product}">
            <button class="btn btn-primary my-3" type="submit" name="id"
                    value=${product.id} formaction="${pageContext.request.contextPath}/secured//edit-product?id=${product.id}">Edit</button>

            <a class="btn btn-danger" role="button" href="${pageContext.request.contextPath}/secured/delete-product?id=${product.id}">Delete</a>
        </c:when>
        <c:otherwise>
